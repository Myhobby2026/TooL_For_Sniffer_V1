/* ==========================================================================
 * usn_wire_crc.c -- CRC32C (Castagnoli, reflected polynomial 0x82F63B78).
 *
 * Reflected table-driven implementation, one byte per iteration. Verified
 * against the RFC 3720 Appendix B test vectors (see
 * tests/unit/wire_crc_test.cpp), which are the standard vectors for this
 * polynomial:
 *   32 bytes of 0x00          -> 0x8a9136aa
 *   32 bytes of 0xff          -> 0x62a8ab43
 *   32 bytes ascending 00..1f -> 0x46dd794e
 *   32 bytes descending 1f..00-> 0x113fdb5c
 *   "123456789"               -> 0xe3069283
 *
 * A slice-by-8 or hardware-CRC variant can replace the body of this file
 * without touching the header, which is why the interface is streaming.
 * ========================================================================== */
...
{
    return crc ^ USN_CRC32C_INIT;
}

uint32_t usn_crc32c(const void* data, size_t length)
{
    return usn_crc32c_final(usn_crc32c_update(USN_CRC32C_INIT, data, length));
}

int usn_crc32c_verify(const void* data, size_t length, uint32_t expected)
{
    return usn_crc32c(data, length) == expected ? 1 : 0;
}
